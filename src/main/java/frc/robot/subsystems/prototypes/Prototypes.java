  ...
  public Prototypes(PrototypeMotor... motors) {
    for (PrototypeMotor motor : motors) {
      ShuffleboardLayout layout =
          Shuffleboard.getTab(PARENT_TAB_NAME)
              .getLayout(motor.name, BuiltInLayouts.kList)
              .withSize(2, 6);

      SendableMotor sendableMotor = new SendableMotor(motor.id);
      // PID constants sublayout
      layout.getLayout(CONSTANTS_SUBLAYOUT_NAME, BuiltInLayouts.kList).add(sendableMotor);

      // percent (voltage) control sublayout
      layout
          .getLayout(PERCENT_SUBLAYOUT_NAME, BuiltInLayouts.kList)
          .add(sendableMotor.profiledPercentCommand());
      layout.getLayout(PERCENT_SUBLAYOUT_NAME).add(sendableMotor.toggleEnable());
      layout
          .getLayout(PERCENT_SUBLAYOUT_NAME)
      ...
      layout.getLayout(POSITION_SUBLAYOUT_NAME).add(sendableMotor.positionRangeCommand());
      layout
          .getLayout(POSITION_SUBLAYOUT_NAME)
          .addDouble("Goal position", () -> sendableMotor.goalPos)
          .withWidget(kNumberSlider)
          .withProperties(
              Map.of("min", sendableMotor.LOWER_POS_LIMIT, "max", sendableMotor.UPPER_POS_LIMIT));
      layout
          .getLayout(POSITION_SUBLAYOUT_NAME)
          .addDouble("Actual position", () -> sendableMotor.actualPos);

      // velocity control sublayout
      layout.getLayout(VELOCITY_SUBLAYOUT_NAME).add(sendableMotor.profiledVelocityCommand());
      layout
          .getLayout(VELOCITY_SUBLAYOUT_NAME)
          .addDouble("Goal velocity", () -> sendableMotor.goalVel)
          .withWidget(kNumberSlider)
          .withProperties(
              Map.of("min", sendableMotor.LOWER_VEL_LIMIT, "max", sendableMotor.UPPER_VEL_LIMIT));
      layout
          .getLayout(VELOCITY_SUBLAYOUT_NAME)
